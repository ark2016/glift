...
gulp.task('compile', () => {
  return gulp
    .src(jsSrcGlobGen(srcPaths, srcIgnore), { base: '.' })
...
gulp.task('update-html-srcs', () => {
  return gulp.src(jsSrcGlobGen(srcPaths, srcIgnore), { base: '.' }).pipe(
    updateHtmlFiles({
...
gulp.task(
  'update-html-compiled',
  gulp.series('compile', () => {
    return gulp.src('./compiled/glift.js').pipe(
      updateHtmlFiles({
        filesGlob: './test/htmltests/*.html',
        outDir: './test/htmltests_gen/',
        header: '<!-- AUTO-GEN-DEPS -->',
...
gulp.task(
  'test',
  gulp.series('update-html-tests', 'update-html-srcs', () => {
...
gulp.task('test-watch', () => {
  return gulp.watch(['src/**/*.js', 'src/**/*_test.js'], gulp.series('test'));
});
...
gulp.task(
  'compile-test',
  gulp.series('update-html-compiled', () => {
    return gulp.src('./test/htmltests_gen/QunitTest.html').pipe(qunit());
  })
);
...
gulp.task('src-gen', () => {
  gulp.src(jsSrcGlobGen(srcPaths, srcIgnore), { base: '.' }).pipe(
    through.obj(
