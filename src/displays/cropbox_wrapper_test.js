(function () {
  ...
  test('For 19x19', function () {
    ...
    deepEqual(
      box.bbox().botRight().x(),
      18,
      'botRight.x() must be 18, for ALL'
    );
    deepEqual(
      box.bbox().botRight().y(),
      18,
      'botRight.y() must be 18, for ALL'
    );
  });
  ...
  test('Cropbox tests for quarter boards', function () {
    ...
    deepEqual(
      tlbox.widthIntersections(),
      12 + overf + ragp,
      'width for TOP LEFT'
    );
    deepEqual(
      tlbox.heightIntersections(),
      11 + overf + ragp,
      'height for TOP LEFT'
    );
    ...
    deepEqual(
      trbox.widthIntersections(),
      12 + overf + ragp,
      'width for TOP RIGHT'
    );
    deepEqual(
      trbox.heightIntersections(),
      11 + overf + ragp,
      'height for TOP RIGHT'
    );

    deepEqual(blbox.bbox().botRight().x(), 11, 'right coord for BOTTOM LEFT');
    deepEqual(blbox.bbox().topLeft().y(), 8, 'top coord for BOTTOM LEFT');
    deepEqual(
      blbox.widthIntersections(),
      12 + overf + ragp,
      'width for BOTTOM LEFT'
    );
    deepEqual(
      blbox.heightIntersections(),
      11 + overf + ragp,
      'height for BOTTOM LEFT'
    );

    deepEqual(brbox.bbox().topLeft().x(), 7, 'left coord for BOTTOM RIGHT');
    deepEqual(brbox.bbox().topLeft().y(), 8, 'top coord for BOTTOM RIGHT');
    deepEqual(
      brbox.widthIntersections(),
      12 + overf + ragp,
      'width for BOTTOM RIGHT'
    );
    deepEqual(
      brbox.heightIntersections(),
      11 + overf + ragp,
      'height for BOTTOM RIGHT'
    );
  });
})();
