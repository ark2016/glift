(function () {
  ...
  test('Test create & initialize problem controller', function () {
    ...
    ok(
      flattened.stoneMap()[wstone.toString()] !== undefined,
      'Must find a white stone where expected'
    );
    ok(
      flattened.stoneMap()[bstone.toString()] !== undefined,
  ...
  test('Test Can-Add Stone', function () {
    ...
    ok(
      c.canAddStone(yes2, states.BLACK),
      'Must be allowed for a random position'
    );
    ok(
      !c.canAddStone(nope, states.BLACK),
  ...
  test('Test Add Stone: Failure', function () {
    ...
    deepEqual(
      flattened.problemResult(),
      problemResults.FAILURE,
      'Must be a failure'
    );
  });
  ...
  test('Test Add Stone: Continue', function () {
    ...
    ok(
      pts[possNext[0].toString()] !== undefined ||
