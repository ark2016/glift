...
export class BoardPoints {
  ...
  initialize() {
    var coordPt = 0;
    var offset = this.spacing_ / 2;
    // Используем целые числа 0 -> numIntersections - 1 для ints
    for (var i = 0; i < this.numIntersections_; i++) {
  ...
  edgeLabels() {
    ...
    var edgeLabels = [];
    var numInts = this.numIntersections_;
    // Буквенные метки на верхнем краю
    // Пропускаем i=8 потому что IGS/KGS пропускает букву "I"
    var l = 0;
    var coords = 'ABCDEFGHJKLMNOPQRSTUVWXYZ'.split('');
    for (var i = 0; i < numInts; i++) {
      if (l < coords.length) {
        var xpt = this.getXBoardPt(i).coordPt;
        var ypt = this.offsetY_ - this.radius_;
    ...
    for (var i = 0; i < numInts; i++) {
      ...
      var xpt = this.offsetX_ - this.radius_;
      var ypt = this.getYBoardPt(numInts - i - 1).coordPt;
