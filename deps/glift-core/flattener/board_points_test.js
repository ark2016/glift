(function () {
  ...
  test('BoardPoints Construction: normal full board.', function () {
    ...
    deepEqual(
      bp.getCoord(new glift.Point(0, 0)).coordPt,
      new glift.Point(half, half),
      'Zero coordPt'
    );
    ...
    deepEqual(
      bp.getCoord(new glift.Point(13, 15)).intPt,
      new glift.Point(13, 15)
    );
    deepEqual(
      bp.getCoord(new glift.Point(13, 15)).coordPt,
  ...
  test('BoardPoints: star points.', function () {
    ...
    deepEqual(bp.starPoints(), [new glift.Point(4, 4)]);
  });
  ...
  test('BoardPoints: drawBoardCoords.', function () {
    ...
    deepEqual(bp.edgeLabels[0], {
      label: '19',
      coordPt: new glift.Point(half, spacing + half),
    });
    deepEqual(bp.edgeLabels[18], {
      label: '1',
      coordPt: new glift.Point(half, 19 * spacing + half),
    });
    deepEqual(bp.edgeLabels[19], {
      label: 'A',
      coordPt: new glift.Point(spacing + half, half),
    });
    deepEqual(bp.edgeLabels[20], {
      label: 'A',
      coordPt: new glift.Point(spacing + half, spacing * 20 + half),
    });
    deepEqual(bp.edgeLabels[19 * 3], {
      label: '19',
      coordPt: new glift.Point(spacing * 20 + half, spacing + half),
    });
    deepEqual(bp.edgeLabels[19 * 4 - 1], {
      label: '1',
      coordPt: new glift.Point(spacing * 20 + half, spacing * 19 + half),
    });

    ok(bp.hasCoord(new glift.Point(0, 0)));
    deepEqual(
      bp.getCoord(new glift.Point(0, 0)).coordPt,
      new glift.Point(spacing + half, spacing + half),
      'Zero coordPt'
    );
  ...
  test('BoardPoints: drawBoardCoords, cropped, padding', function () {
    ...
    deepEqual(bp.edgeLabels[0], {
      label: '11',
      coordPt: new glift.Point(pads + half, pads + spacing + half),
    });

    deepEqual(bp.edgeLabels[11], {
      label: 'H',
      coordPt: new glift.Point(pads + spacing + half, pads + half),
    });
  ...
  test('BoardPoints: offsetPt', function () {
    ...
    deepEqual(
      bp.getCoord(new glift.Point(18, 18)).coordPt,
      new glift.Point(
        18 * spacing + half + pt.x(),
        18 * spacing + half + pt.y()
      ),
      '18,18 coordPt'
    );
  });
