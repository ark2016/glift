...
export class Board {
    ...
    for (var i = 0; i < this.boardSize; i++) {
      for (var j = 0; j < this.boardSize; j++) {
        ...
        this.boardArray_[i][j] = intersection.create(
            pt, enums.states.EMPTY, undefined, undefined, this.boardSize);
      }
    }
    return this;
  }
   ...
   *
   * @param {!Point} point
   * @param {!enums.states} color BLACK, WHITE или EMPTY
   * @return {!Board} this
   */
  addStone(point, color) {
    this.getIntBoardPt(point).setStone(
        color === enums.states.BLACK ? symbols.BSTONE :
        color === enums.states.WHITE ? symbols.WSTONE : symbols.EMPTY);
    return this;
  }
  ...
  addMark(point, mark, opt_textLabel) {
    var intersection = this.getIntBoardPt(point);
