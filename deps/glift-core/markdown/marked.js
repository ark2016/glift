 ...
 * @param {(!marked.Options|Function)=} opt_options Marked options or
 *    callback
 * @param {Function=} opt_callback
 */
export function marked(src, opt_options, opt_callback) {
  /** @type {Function} */
  var callback;
  /** @type {marked.Options|undefined} */
  var opt;
  ...
  if (opt_callback || typeof opt_options === 'function') {
    ...
    opt = /** @type {marked.Options} */ (
      merge({}, marked.defaults, opt_options || {})
    );
  ...
  try {
    ...
    if (opt_options) {
      opt = merge({}, marked.defaults, opt_options);
    }
    return Parser.parse(Lexer.lex(src, opt), opt);
  } catch (e) {
    e.message += '\nPlease report this to https://github.com/kashomon/glift.';
    if ((opt_options || marked.defaults).silent) {
      return (
...
var Blocker = function () {
  ...
  this.bullet = /(?:[*+-]|\d+\.)/;
  this.item = /^( *)(bull) [^\n]*(?:\n(?!\1bull )[^\n]*)*/;
  this.item = replace(this.item, 'gm')(/bull/g, this.bullet)();
  this.list = replace(this.list)(/bull/g, this.bullet)(
    'hr',
    '\\n+(?=\\1?(?:[-*_] *){3,}(?:\\n+|$))'
  )('def', '\\n+(?=' + this.def.source + ')')();
  this.blockquote = replace(this.blockquote)('def', this.def)();
  this._tag =
    '(?!(?:' +
    'a|em|strong|small|s|cite|q|dfn|abbr|data|time|code' +
    '|var|samp|kbd|sub|sup|i|b|u|mark|ruby|rt|rp|bdi|bdo' +
    '|span|br|wbr|ins|del|img)\\b)\\w+(?!:/|[^\\w\\s@]*@)\\b';
  this.html = replace(this.html)('comment', /<!--[\s\S]*?-->/)(
    'closed',
    /<(tag)[\s\S]+?<\/\1>/
  )('closing', /<tag(?:"[^"]*"|'[^']*'|[^'">])*?>/)(/tag/g, this._tag)();
  this.paragraph = replace(this.paragraph)('hr', this.hr)(
    'heading',
    this.heading
  )('lheading', this.lheading)('blockquote', this.blockquote)(
    'tag',
    '<' + this._tag
  )('def', this.def)();
 ...
 * @constructor
 *
 * @param {marked.Options=} opt_options
 */
marked.Lexer = function (opt_options) {
  this.tokens = [];
  this.tokens.links = {};
  this.options = opt_options || marked.defaults;
 ...
 * @param {marked.Options=} opt_options
 *
 * @return {!Array<!marked.Token>} Array of tokens
 */
 ...
 * @constructor
 */
marked.InlineLexer = function (links, options) {
 ...
 * @param {marked.Options=} opt_options
 */
marked.Renderer = function (opt_options) {
 ...
 * @param {marked.Options=} opt_options
 */
marked.Parser = function (opt_options) {
  this.tokens = [];
  this.token = null;
  this.options = opt_options || marked.defaults;
...
/**
 * Static Parse Method
 * @param {!Array<marked.Token>} src Array of tokens.
 * @param {marked.Options=} opt_options Optional marked options
 */
 ...
 *  renderer: (!marked.Renderer|undefined),
