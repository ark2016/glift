(function () {
  ...
  test('Capture -- center', function () {
    ...
    deepEqual(
      result.captures[0].toString(),
      glift.util.point(1, 2).toString(),
      'must have captured the white stone'
    );
    deepEqual(
      test_goban.getStone(glift.util.point(1, 2)),
      EMPTY,
      'Must be removed'
    );
  });
  ...
  test('Init from MoveTree', function () {
    ...
    deepEqual(
      goban.getStone(expPoint_pm),
      glift.enums.states.WHITE,
      'Must get WHITE'
    );
    deepEqual(
      goban.getStone(expPoint_w),
      glift.enums.states.WHITE,
      'Must get WHITE'
    );
    deepEqual(
      goban.getStone(expPoint_b),
      glift.enums.states.BLACK,
      'Must get BLACK'
    );
    deepEqual(
      goban.getStone(glift.util.point(6, 6)),
      glift.enums.states.EMPTY,
      'Must get EMPTY'
    );
  });
  ...
  test('Load stones and apply clear locations from movetree', function () {
    ...
    mt.properties()
      ...
      .add('AW', 'cc');
    mt.addNode();
    mt.properties().add('B', 'cb'); // should capture 'bb'
    mt.addNode();
    mt.properties()
      .add('AE', 'ac')
      .add('AE', 'ad')
