(function () {
  ...
  test('Test that property retrieval for multiple props works', function () {
    ...
    deepEqual(
      mt.properties().getAllValues('AB')[1],
      'qa',
      'Should get the second property'
    );
    deepEqual(
      mt.properties().getAllValues('AW').toString(),
  ...
  test(
    'Edge case of moving up: only one move left - works.' +
      'In other words, dont remove the last move',
    function () {
      var mt = movetree.getFromSgf(sgfs.easy);
      mt.moveUp();
      deepEqual(mt.node().getNodeNum(), 0, 'move num');
  ...
  test('Test that deleting a property works', function () {
    var mt = movetree.getFromSgf(sgfs.veryeasy);
    deepEqual(
      mt.properties().getOneValue('AP'),
      'CGoban:3',
      'should get the AP prop'
    );
    deepEqual(
      mt.properties().remove('AP')[0],
      'CGoban:3',
      'should delete the prop'
    );
  ...
  test('Test that adding properties works', function () {
    ...
    movt.properties().set('C', 'foo').add('EV', 'tourny');
    deepEqual(
      movt.properties().getOneValue('C'),
      'foo',
      'Should get the correct comment'
    );
    deepEqual(
      movt.properties().getOneValue('EV'),
      'tourny',
  ...
  test('Adding Nodes Works', function () {
    ...
    movt.addNode().properties().add('C', '1.0');
    movt.moveUp().addNode().properties().add('C', '1.1');
    ...
    deepEqual(movt.node().getNodeNum(), 1, 'Should get the move num');
    deepEqual(movt.node().getVarNum(), 1, 'Should get the var num');
  });
  ...
  test('Get Property as a Point', function () {
    ...
    deepEqual(
      movt.properties().getAsPoint('B').x(),
      15,
      'Should get and covert the x coord correctly'
    );
    deepEqual(
      movt.properties().getAsPoint('B').y(),
      1,
  ...
  test('onMainline', function () {
    ...
    var sgf =
      '(;GM[1];B[aa];W[ab];B[ac];W[ad];B[ae];W[af]' +
      '(;B[ag];W[ah];B[ai];W[aj])' +
      '(;B[ah];W[ai];B[aj];W[ak]))';
  ...
  test('getCurrentPlayer -- complex problem', function () {
    var states = glift.enums.states;
    var mt = glift.rules.movetree.getFromSgf(testdata.sgfs.complexproblem);
  ...
  test('Rebase Test', function () {
    var initPos = [0];
    var sgf =
      '(;GM[1]AW[jj][jk][jl]AB[kk][kl]C[foo];' +
