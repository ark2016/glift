(function () {
  ...
  test('Parse warning on unknown props', function () {
    ...
    ok(
      parseErrors[0].indexOf('Parsing Warning') !== -1,
      'Actual msg: ' + parseErrors[0]
    );
    ok(
      parseErrors[1].indexOf('The property [ZZ] is not valid') !== -1,
      'Actual msg: ' + parseErrors[1]
    );

    ok(
      parseErrors[2].indexOf('Parsing Warning') !== -1,
      'Actual msg: ' + parseErrors[2]
    );
    ok(
      parseErrors[3].indexOf('The property [Z] is not valid') !== -1,
  ...
  test('Fail to process Metadata at root: bad JSON', function () {
    ...
    ok(
      /Tried to parse property GC/.test(parseErrors[0]),
      'Regex does not match: ' + parseErrors[0]
    );

    deepEqual(
      mt.properties().getOneValue('GC'),
      'foo: bar',
  ...
  test('Rectangles of stones', function () {
    var sgf =
      '(;GM[1]AB[pb:sb][oc][sc][nd][oe:pe]AW[pa:qa]' +
